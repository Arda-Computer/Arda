...
public class old_PositionManager : MonoBehaviour
{
    ...
    //if url changes
       public async void MainWebView_UrlChanged(object sender, UrlChangedEventArgs eventArgs) {
            ...
            this.gameObject.transform.parent.GetComponent<old_GroupManager>().AddToList(currentURL, currentImagebytes);
        ...
        async void Convert(){
            ...
            CanvasWebViewPrefab.gameObject.SetActive(true);
            await CanvasWebViewPrefab.WaitUntilInitialized(); //load before doing anything
            CanvasWebViewPrefab.WebView.LoadUrl(this.gameObject.transform.parent.GetComponent<old_GroupManager>().lastURL);
            CanvasWebViewPrefab.WebView.UrlChanged += MainWebView_UrlChanged;//if url changes

            this.gameObject.transform.parent.GetComponent<old_GroupManager>().Display(savedPos);
